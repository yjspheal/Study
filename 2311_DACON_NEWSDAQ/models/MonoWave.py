...
class MonoWaveDown2(MonoWave):
    def __init__(self, *args, custom_date=None, idx_end=None, **kwargs):
        super().__init__(*args, **kwargs)

        # custom_date로부터 low를 설정
        if custom_date is not None and custom_date >= self.idx_start:
            self.low = self.lows_arr[custom_date]
            self.low_idx = custom_date
            self.date_end = self.dates_arr[custom_date]
            self.idx_end = custom_date
        else:
            self.low, self.low_idx = self.find_end()
            self.date_end = self.dates_arr[self.low_idx]
            self.idx_end = self.low_idx
        ...
        self.high = self.highs_arr[self.idx_start]
        self.high_idx = self.idx_start
        self.idx_end = idx_end  # idx_end 추가
