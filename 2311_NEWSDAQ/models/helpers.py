...
def plot_duowave(df, monowave1, monowave2, title: str = ''):
    data = go.Ohlc(x=df['Date'],
                   open=df['Open'],
                   high=df['High'],
                   low=df['Low'],
    ...
    # 첫 번째 Monowave 그래프 설정
    monowaves1 = go.Scatter(x=monowave1.dates,
                            y=monowave1.points,
                            mode='lines+markers+text',
                            textposition='middle right',
                            textfont=dict(size=15, color='#2c3035'),
    ...
    # 두 번째 Monowave 그래프 설정
    monowaves2 = go.Scatter(x=monowave2.dates,
                            y=monowave2.points,
                            mode='lines+markers+text',
                            textposition='middle right',
                            textfont=dict(size=15, color='#FF5733'),  # 다른 색상 선택
                            line=dict(
                                color=('rgb(255, 87, 51)'),  # 다른 색상 선택
                                width=3),
                            name='Monowave 2')

    layout = dict(title=title)
    fig = go.Figure(data=[data, monowaves1, monowaves2], layout=layout)
    fig.update(layout_xaxis_rangeslider_visible=False)
...
def plot_duowave_today(df, monowave1, monowave2, title: str = ''):
    data = go.Ohlc(x=df['Date'],
                   open=df['Open'],
                   high=df['High'],
                   low=df['Low'],
