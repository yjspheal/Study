...
def youtube_crawling():
    ...
    raw_data = xl.load_workbook('올리브영랭크100위화장품정보_상세페이지.xlsx')
...
def excel_add_youtube():
    ...
    raw_datad = xl.load_workbook('올리브영랭크100위화장품정보_상세페이지.xlsx')
...
def make_top20():
    ...
    try: # 결과 저장할 엑셀 생성
        top20d = pd.ExcelWriter('올리브영_진정_상위20.xlsx', engine='openpyxl')
    ...
    # 만든 파일 활성화
    top20d = xl.load_workbook('올리브영_진정_상위20.xlsx')
    top20 = top20d.active
    
    # 유튜브데이터까지 들어있는 엑셀을 load후 활성화
    ol_yud = xl.load_workbook('올리브영랭크100위화장품정보+유튜브데이터.xlsx')
    ol_yu = ol_yud.active
    
    # 트렌드 지수 들어있는 엑셀 load 후 활성화
    trendd = xl.load_workbook('./트렌드 지수(아이템 스카우트)/트렌드지수 크롤링(브랜드).xlsx')
    trend = trendd.active
